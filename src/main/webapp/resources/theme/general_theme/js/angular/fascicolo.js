var fascicoloApp=angular.module("fascicoloApp",['ngTouch', 'ui.grid', 'ui.grid.pagination','ui.grid.selection','ui.date', 'ui.grid.exporter'])
...
pazienteService.selectedEntity.show=false;ambulatorioService.selectedEntity.show=false;}
$scope.addNew= function()
{
fascicoloService.setSelectedEntity(null);
fascicoloService.setEntityList(null);
fascicoloService.selectedEntity.show=true;
pazienteService.selectedEntity.show=false;ambulatorioService.selectedEntity.show=false;$('#fascicoloTabs li:eq(0) a').tab('show');
};
...
{
if (!$scope.fascicoloDetailForm.$valid) return; 
pazienteService.selectedEntity.show=false;ambulatorioService.selectedEntity.show=false;fascicoloService.update().then(function successCallback(response) { 
$scope.search();
},function errorCallback(response) { 
alert("error");
return; 
...
$scope.del=function()
{
nullService.selectedEntity.fascicolo=null;
...
{ name: 'paziente.pazienteId', displayName: 'paziente'},
{ name: 'ambulatorio.ambulatorioId', displayName: 'ambulatorio'},
{ name: 'tipoIntervento'} 
]
,data: fascicoloService.entityList
 };
$scope.fascicoloGridOptions.onRegisterApi = function(gridApi){
gridApi.selection.on.rowSelectionChanged($scope,function(row){
pazienteService.selectedEntity.show=false;ambulatorioService.selectedEntity.show=false;if (row.isSelected)
{
fascicoloService.setSelectedEntity(row.entity);
...
fascicoloService.selectedEntity.paziente=response.data;
fascicoloService.initPazienteList().then(function(response) {
fascicoloService.childrenList.pazienteList=response.data;
});
},function errorCallback(response) { 
alert("error");
...
fascicoloService.selectedEntity.paziente=pazienteService.selectedEntity;
...
{
pazienteService.selectedEntity.show=false;
fascicoloService.selectedEntity.paziente=null;
pazienteService.setSelectedEntity(null);
$scope.updateParent();
};
$scope.del=function()
