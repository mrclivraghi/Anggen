    ...
    extends JpaRepository<it.anggen.model.security.User, java.lang.Long>
{
    ...
    @Query("select u from User u where  (:userId is null or cast(:userId as string)=cast(u.userId as string)) and (:password is null or :password='' or cast(:password as string)=u.password) and (:username is null or :username='' or cast(:username as string)=u.username) and (:enabled is null or cast(:enabled as string)=cast(u.enabled as string)) and (:role in elements(u.roleList)  or :role is null) ")
    public List<it.anggen.model.security.User> findByUserIdAndPasswordAndUsernameAndEnabledAndRole(
