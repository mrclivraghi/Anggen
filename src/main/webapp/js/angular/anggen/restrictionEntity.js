var restrictionEntityApp=angular.module("restrictionEntityApp",['ngFileUpload','ngTouch', 'ui.grid', 'ui.grid.pagination','ui.grid.selection','ui.date', 'ui.grid.exporter'])
...
roleService.selectedEntity.show=false;
roleService.selectedEntity.projectList.push(projectService.selectedEntity);
roleService.insert().then(function successCallBack(response) { 
projectService.selectedEntity.roleList.push(response.data);
},function errorCallback(response) { 
AlertError.init({selector: "#alertError"});
AlertError.show("Si è verificato un errore");
return; 
...
for (i=0; i<projectService.selectedEntity.roleList.length; i++)

{

if (projectService.selectedEntity.roleList[i].roleId==roleService.selectedEntity.roleId)

projectService.selectedEntity.roleList.splice(i,1);

}

projectService.selectedEntity.roleList.push(roleService.selectedEntity);
...
roleService.setSelectedEntity(null);
projectService.initRoleList().then(function(response) {
projectService.childrenList.roleList=response.data;
});
},function errorCallback(response) { 
AlertError.init({selector: "#alertError"});
AlertError.show("Si è verificato un errore");
