    ...
    extends CrudRepository<it.generated.anggen.model.entity.Tab, java.lang.Long>
{
    ...
    @Query("select t from Tab t where  (:name is null or :name='' or cast(:name as string)=t.name) and (:tabId is null or cast(:tabId as string)=cast(t.tabId as string)) and (:enumField in elements(t.enumFieldList)  or :enumField is null) and (:relationship in elements(t.relationshipList)  or :relationship is null) and (:field in elements(t.fieldList)  or :field is null) and (:entity=t.entity or :entity is null) ")
    public List<it.generated.anggen.model.entity.Tab> findByNameAndTabIdAndEnumFieldAndRelationshipAndFieldAndEntity(
