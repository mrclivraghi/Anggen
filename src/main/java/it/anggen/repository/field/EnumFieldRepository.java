    ...
    extends CrudRepository<it.anggen.model.field.EnumField, java.lang.Long>
{
    ...
    @Query("select e from EnumField e where  (:enumFieldId is null or cast(:enumFieldId as string)=cast(e.enumFieldId as string)) and (:name is null or :name='' or cast(:name as string)=e.name) and (:priority is null or cast(:priority as string)=cast(e.priority as string)) and (:tab=e.tab or :tab is null) and (:annotation in elements(e.annotationList)  or :annotation is null) and (:entity=e.entity or :entity is null) and (:enumEntity=e.enumEntity or :enumEntity is null) ")
    public List<it.anggen.model.field.EnumField> findByEnumFieldIdAndNameAndPriorityAndTabAndAnnotationAndEntityAndEnumEntity(
