    ...
    extends JpaRepository<it.anggen.model.security.RestrictionEntity, java.lang.Long>
{
    ...
    @Query("select r from RestrictionEntity r where  (:restrictionEntityId is null or cast(:restrictionEntityId as string)=cast(r.restrictionEntityId as string)) and (:canDelete is null or cast(:canDelete as string)=cast(r.canDelete as string)) and (:canSearch is null or cast(:canSearch as string)=cast(r.canSearch as string)) and (:canUpdate is null or cast(:canUpdate as string)=cast(r.canUpdate as string)) and (:canCreate is null or cast(:canCreate as string)=cast(r.canCreate as string)) and (:role=r.role or :role is null) and (:entity=r.entity or :entity is null) ")
    public List<it.anggen.model.security.RestrictionEntity> findByRestrictionEntityIdAndCanDeleteAndCanSearchAndCanUpdateAndCanCreateAndRoleAndEntity(
