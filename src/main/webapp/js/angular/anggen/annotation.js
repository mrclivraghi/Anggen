...
{
if (!$scope.annotationDetailForm.$valid) return; 
enumFieldService.selectedEntity.show=false;tabService.selectedEntity.show=false;relationshipService.selectedEntity.show=false;entityService.selectedEntity.show=false;fieldService.selectedEntity.show=false;restrictionFieldService.selectedEntity.show=false;roleService.selectedEntity.show=false;restrictionEntityGroupService.selectedEntity.show=false;entityGroupService.selectedEntity.show=false;projectService.selectedEntity.show=false;enumEntityService.selectedEntity.show=false;enumValueService.selectedEntity.show=false;restrictionEntityService.selectedEntity.show=false;userService.selectedEntity.show=false;annotationAttributeService.selectedEntity.show=false;annotationService.update().then(function successCallback(response) { 
$scope.search();
},function errorCallback(response) { 
AlertError.init({selector: "#alertError"});
AlertError.show("Si è verificato un errore");
...
annotationService.selectedEntity.tabList.push(response.data);
},function errorCallback(response) { 
AlertError.init({selector: "#alertError"});
AlertError.show("Si è verificato un errore");
return; 
...
for (i=0; i<annotationService.selectedEntity.tabList.length; i++)

{

if (annotationService.selectedEntity.tabList[i].tabId==tabService.selectedEntity.tabId)

annotationService.selectedEntity.tabList.splice(i,1);
...
tabService.setSelectedEntity(null);
annotationService.initTabList().then(function(response) {
annotationService.childrenList.tabList=response.data;
});
},function errorCallback(response) { 
AlertError.init({selector: "#alertError"});
AlertError.show("Si è verificato un errore");
