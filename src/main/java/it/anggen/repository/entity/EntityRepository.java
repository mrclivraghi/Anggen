    ...
    extends JpaRepository<it.anggen.model.entity.Entity, java.lang.Long>
{
    ...
    @Query("select e from Entity e where  (:entityId is null or cast(:entityId as string)=cast(e.entityId as string)) and (:enableRestrictionData is null or cast(:enableRestrictionData as string)=cast(e.enableRestrictionData as string)) and (:generateFrontEnd is null or cast(:generateFrontEnd as string)=cast(e.generateFrontEnd as string)) and (:cache is null or cast(:cache as string)=cast(e.cache as string)) and (:descendantMaxLevel is null or cast(:descendantMaxLevel as string)=cast(e.descendantMaxLevel as string)) and (:disableViewGeneration is null or cast(:disableViewGeneration as string)=cast(e.disableViewGeneration as string)) and (:name is null or :name='' or cast(:name as string)=e.name) and (:securityType is null or cast(:securityType as string)=cast(e.securityType as string)) and (:field in elements(e.fieldList)  or :field is null) and (:enumField in elements(e.enumFieldList)  or :enumField is null) and (:tab in elements(e.tabList)  or :tab is null) and (:entityGroup=e.entityGroup or :entityGroup is null) and (:restrictionEntity in elements(e.restrictionEntityList)  or :restrictionEntity is null) and (:relationship in elements(e.relationshipList)  or :relationship is null) ")
    public List<it.anggen.model.entity.Entity> findByEntityIdAndEnableRestrictionDataAndGenerateFrontEndAndCacheAndDescendantMaxLevelAndDisableViewGenerationAndNameAndSecurityTypeAndFieldAndEnumFieldAndTabAndEntityGroupAndRestrictionEntityAndRelationship(
