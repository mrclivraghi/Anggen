    ...
    extends JpaRepository<it.anggen.model.field.Field, java.lang.Long>
{
    ...
    @Query("select f from Field f where  (:fieldId is null or cast(:fieldId as string)=cast(f.fieldId as string)) and (:priority is null or cast(:priority as string)=cast(f.priority as string)) and (:name is null or :name='' or cast(:name as string)=f.name) and (:fieldType is null or cast(:fieldType as string)=cast(f.fieldType as string)) and (:restrictionField in elements(f.restrictionFieldList)  or :restrictionField is null) and (:tab=f.tab or :tab is null) and (:entity=f.entity or :entity is null) and (:annotation in elements(f.annotationList)  or :annotation is null) ")
    public List<it.anggen.model.field.Field> findByFieldIdAndPriorityAndNameAndFieldTypeAndRestrictionFieldAndTabAndEntityAndAnnotation(
