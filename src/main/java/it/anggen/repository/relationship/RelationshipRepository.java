    ...
    extends JpaRepository<it.anggen.model.relationship.Relationship, java.lang.Long>
{
    ...
    @Query("select r from Relationship r where  (:relationshipId is null or cast(:relationshipId as string)=cast(r.relationshipId as string)) and (:priority is null or cast(:priority as string)=cast(r.priority as string)) and (:name is null or :name='' or cast(:name as string)=r.name) and (:relationshipType is null or cast(:relationshipType as string)=cast(r.relationshipType as string)) and (:entity=r.entity or :entity is null) and (:entityTarget=r.entityTarget or :entityTarget is null) and (:tab=r.tab or :tab is null) and (:annotation in elements(r.annotationList)  or :annotation is null) ")
    public List<it.anggen.model.relationship.Relationship> findByRelationshipIdAndPriorityAndNameAndRelationshipTypeAndEntityAndEntityAndTabAndAnnotation(
