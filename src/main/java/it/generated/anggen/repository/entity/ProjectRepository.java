    ...
    extends CrudRepository<it.generated.anggen.model.entity.Project, java.lang.Integer>
{
    ...
    @Query("select p from Project p where  (:name is null or :name='' or cast(:name as string)=p.name) and (:projectId is null or cast(:projectId as string)=cast(p.projectId as string)) and (:entityId is null or cast(:entityId as string)=cast(p.entityId as string)) and (:entityGroup in elements(p.entityGroupList)  or :entityGroup is null) ")
    public List<it.generated.anggen.model.entity.Project> findByNameAndProjectIdAndEntityIdAndEntityGroup(
