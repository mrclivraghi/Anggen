...
public interface RestrictionEntityGroupRepository
    extends CrudRepository<it.anggen.model.security.RestrictionEntityGroup, java.lang.Long>
{
    ...
    @Query("select r from RestrictionEntityGroup r where  (:restrictionEntityGroupId is null or cast(:restrictionEntityGroupId as string)=cast(r.restrictionEntityGroupId as string)) and (:canCreate is null or cast(:canCreate as string)=cast(r.canCreate as string)) and (:canSearch is null or cast(:canSearch as string)=cast(r.canSearch as string)) and (:canDelete is null or cast(:canDelete as string)=cast(r.canDelete as string)) and (:canUpdate is null or cast(:canUpdate as string)=cast(r.canUpdate as string)) and (:entityGroup=r.entityGroup or :entityGroup is null) and (:role=r.role or :role is null) ")
    public List<it.anggen.model.security.RestrictionEntityGroup> findByRestrictionEntityGroupIdAndCanCreateAndCanSearchAndCanDeleteAndCanUpdateAndEntityGroupAndRole(
