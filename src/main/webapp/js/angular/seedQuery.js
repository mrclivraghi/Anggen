...
mountainService.selectedEntity.show=false;$('#seedQueryTabs li:eq(0) a').tab('show');
};
...
{
if (!$scope.seedQueryDetailForm.$valid) return; 
mountainService.selectedEntity.show=false;seedService.update().then(function successCallback(response) { 
$scope.search();
},function errorCallback(response) { 
alert("error");
return; 
...
mountainService.selectedEntity.show=false;if (row.isSelected)
{
seedService.setSelectedEntity(row.entity);
...
seedService.setSelectedEntity(response);
if (toDo != null)
toDo();
},function errorCallback(response) {      
alert("error");
...
seedService.selectedEntity.mountainList.push(response.data);
},function errorCallback(response) { 
alert("error");
return; 
});
...
for (i=0; i<seedService.selectedEntity.mountainList.length; i++)

{

if (seedQueryService.selectedEntity.mountainList[i].mountainId==mountainService.selectedEntity.mountainId)

seedQueryService.selectedEntity.mountainList.splice(i,1);

}

seedQueryService.selectedEntity.mountainList.push(mountainService.selectedEntity);
...
{
mountainService.selectedEntity.show=false;
for (i=0; i<seedQueryService.selectedEntity.mountainList.length; i++)
{
if (seedQueryService.selectedEntity.mountainList[i].mountainId==mountainService.selectedEntity.mountainId)
seedQueryService.selectedEntity.mountainList.splice(i,1);
}
