<head><title>test order</title><script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/angularjs/1.3.14/angular.min.js"></script><script>angular.module("photoApp",[])
...
$http.put("../photo/",photoService.selectedEntity)
...
{
if (!$scope.photoDetailForm.$valid) return; 
seedQueryService.selectedEntity.show=false;mountainService.selectedEntity.show=false;$http.post("../photo/",photoService.selectedEntity)
.success( function(data) {
$scope.search();
})
.error(function() { 
...
$scope.del=function()
{
var url="../photo/"+photoService.selectedEntity.photoId;
$http["delete"](url)
.success( function(data) {
photoService.setSelectedEntity(null);
$scope.search();
...
{
$http.post("../photo/",photoService.selectedEntity)
.then(
function(response) {
...
;</script></head><body ng-app="photoApp"><div ng-controller="photoController"><form id="photoSearchBean"><p>photoId</p><input type="text" ng-model="searchBean.photoId" ng-readonly="false" name="photoId"/><p>url</p><input type="text" ng-model="searchBean.url" ng-readonly="false" name="url"/><p>social</p><input type="text" ng-model="searchBean.social" ng-readonly="false" name="social"/><p>date</p><input type="date" ng-model="searchBean.date" ng-readonly="false" name="date"/><p>status</p><input type="text" ng-model="searchBean.status" ng-readonly="false" name="status"/><p>socialId</p><input type="text" ng-model="searchBean.socialId" ng-readonly="false" name="socialId"/><p>relatedPost</p><input type="text" ng-model="searchBean.relatedPost" ng-readonly="false" name="relatedPost"/><p>seedQuery</p><select ng-model="searchBean.seedQuery.seedQueryId" ng-options="seedQuery.seedQueryId as  seedQuery.seedKeyword for seedQuery in childrenList.seedQueryList" enctype="UTF-8"></select><button ng-click="addNew()">Add new</button><button ng-click="search()">Find</button><button ng-click="reset()">Reset</button></form><form id="photoList" ng-if="entityList.length&gt;0" enctype="UTF-8"><p>LISTA</p><ul><li ng-repeat="entity in entityList" ng-click="showEntityDetail($index)"><p>{{$index}} 
{{entity.photoId}}
{{entity.url}}
{{entity.social}}
