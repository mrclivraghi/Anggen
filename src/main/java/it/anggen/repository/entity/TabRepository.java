    ...
    extends JpaRepository<it.anggen.model.entity.Tab, java.lang.Long>
{
    ...
    public List<it.anggen.model.entity.Tab> findAll();
    ...
    public List<it.anggen.model.entity.Tab> findByAddDate(java.lang.String addDate);

    public List<it.anggen.model.entity.Tab> findByModDate(java.lang.String modDate);

    public List<it.anggen.model.entity.Tab> findByEntity(it.anggen.model.entity.Entity entity);

    @Query("select t from Tab t where  (:tabId is null or cast(:tabId as string)=cast(t.tabId as string)) and (:name is null or :name='' or cast(:name as string)=t.name) and (:addDate is null or cast(:addDate as string)=cast(date(t.addDate) as string)) and (:modDate is null or cast(:modDate as string)=cast(date(t.modDate) as string)) and (:entity=t.entity or :entity is null) and (:field in elements(t.fieldList)  or :field is null) and (:enumField in elements(t.enumFieldList)  or :enumField is null) and (:relationship in elements(t.relationshipList)  or :relationship is null) ")
    public List<it.anggen.model.entity.Tab> findByTabIdAndNameAndAddDateAndModDateAndEntityAndFieldAndEnumFieldAndRelationship(
