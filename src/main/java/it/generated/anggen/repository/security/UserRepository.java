    ...
    extends CrudRepository<it.generated.anggen.model.security.User, java.lang.Long>
{
    ...
    @Query("select u from User u where  (:enabled is null or cast(:enabled as string)=cast(u.enabled as string)) and (:password is null or :password='' or cast(:password as string)=u.password) and (:username is null or :username='' or cast(:username as string)=u.username) and (:userId is null or cast(:userId as string)=cast(u.userId as string)) and (:role in elements(u.roleList)  or :role is null) ")
    public List<it.generated.anggen.model.security.User> findByEnabledAndPasswordAndUsernameAndUserIdAndRole(
