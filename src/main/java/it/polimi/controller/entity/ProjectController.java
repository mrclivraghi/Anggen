...
return ResponseEntity.status(org.springframework.http.HttpStatus.FORBIDDEN).build(); 
    ...
    public ResponseEntity deleteProjectById(
        @PathVariable
        String projectId) {
        if (!securityService.hasPermission(it.polimi.model.entity.Project.staticEntityId, RestrictionType.DELETE)) 
return ResponseEntity.status(org.springframework.http.HttpStatus.FORBIDDEN).build(); 
    ...
    public ResponseEntity insertProject(
        ...
        if (!securityService.hasPermission(it.polimi.model.entity.Project.staticEntityId, RestrictionType.INSERT)) 
return ResponseEntity.status(org.springframework.http.HttpStatus.FORBIDDEN).build(); 
    ...
    public ResponseEntity updateProject(
        @org.springframework.web.bind.annotation.RequestBody
        it.polimi.model.entity.Project project) {
        if (!securityService.hasPermission(it.polimi.model.entity.Project.staticEntityId, RestrictionType.UPDATE)) 
return ResponseEntity.status(org.springframework.http.HttpStatus.FORBIDDEN).build(); 
    ...
    private void getSecurityMapping(it.polimi.model.entity.Project project) {
        if (project.getEntityGroupList()!=null && !securityService.hasPermission(it.polimi.model.entity.EntityGroup.staticEntityId, RestrictionType.SEARCH) )
