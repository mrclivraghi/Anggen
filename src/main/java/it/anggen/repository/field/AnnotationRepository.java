    ...
    extends CrudRepository<it.anggen.model.field.Annotation, java.lang.Long>
{
    ...
    @Query("select a from Annotation a where  (:annotationId is null or cast(:annotationId as string)=cast(a.annotationId as string)) and (:annotationType is null or cast(:annotationType as string)=cast(a.annotationType as string)) and (:enumField=a.enumField or :enumField is null) and (:relationship=a.relationship or :relationship is null) and (:field=a.field or :field is null) and (:annotationAttribute in elements(a.annotationAttributeList)  or :annotationAttribute is null) ")
    public List<it.anggen.model.field.Annotation> findByAnnotationIdAndAnnotationTypeAndEnumFieldAndRelationshipAndFieldAndAnnotationAttribute(
