    ...
    extends JpaRepository<it.anggen.model.generation.GenerationRun, java.lang.Integer>
{
    ...
    @Query("select g from GenerationRun g where  (:generationRunId is null or cast(:generationRunId as string)=cast(g.generationRunId as string)) and (:status is null or cast(:status as string)=cast(g.status as string)) and (:startDate is null or cast(:startDate as string)=cast(date(g.startDate) as string)) and (:endDate is null or cast(:endDate as string)=cast(date(g.endDate) as string)) and (:project=g.project or :project is null) ")
    public List<it.anggen.model.generation.GenerationRun> findByGenerationRunIdAndStatusAndStartDateAndEndDateAndProject(
