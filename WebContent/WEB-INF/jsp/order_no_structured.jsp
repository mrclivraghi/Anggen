...
<title>No Scope Controller</title>
		...
		.service("orderService", function(){
			...
			this.setOrderList= function(data) {
				...
				for (i=0; i<data.length; i++)
				{
					...
					tempOrder.placeList=data[i].placeList;
	...
	.error(function() { alert("error");});
		...
		.controller("orderListController", function($scope,orderService,dateFilter,personService,placeService)
				{
					...
					$scope.showPlaceListDetail = function(index)
					{
						$scope.refreshForm(index);
						placeService.parentIndex=index;
						if ($scope.orderList[index].placeList!=null)
						{
							placeService.placeList=$scope.orderList[index].placeList;
						}
						else
							placeService.placeList= new Object();
						placeService.showEntity=true;
					};
		...
		.service("placeService", function()
					...
					this.showEntity=false;
					this.placeList= null;
					this.setPlaceList = function (newPlaceList)
					{
		...
		{
			$scope.placeList=placeService.placeList;
			$scope.showEntity= function ()
			{
				$scope.placeList=placeService.placeList;
				return placeService.showEntity;
			};
			$scope.closeEntity= function ()
			{
				placeService.showEntity=false;
			};
			...
			$scope.updateParent = function(toDo)
			{
				...
				.success(
						function(data) {
							...
							orderService.orderList[personService.parentIndex]=data;
							orderService.orderForm=data;
							placeService.placeList=orderService.orderForm.placeList;
							if (toDo!=null)
								toDo();
						}).error(function() {
							alert("error");
...
<div ng-app="orderApp">
		...
		<div ng-controller="orderListController">
			<ul>
				<li ng-repeat="order in orderList" ><p ng-click="refreshForm($index)">{{order.orderId}}-{{order.name}}-{{order.timeslotDate | date: 'dd-MM-yyyy'}} 
				<div ng-click="showPersonDetail($index)">
				<div ng-show="order.person!=null">
					Persona : {{order.person.personId}}
				</div>
				<div ng-show="order.person==null">
					Add Person
				</div>
				...
				<div ng-click="showPlaceListDetail($index)">
				<div ng-show="order.placeList!=null">
					Places : # {{order.placeList.length}}
				</div>
				<div ng-show="order.placeList==null">
					Add Place
				</div>
		...
		<div ng-controller="placeController" ng-show="showEntity()">
			<button ng-click="closeEntity()">X</button><br/>
			<ul>
				<li ng-repeat="place in placeList">
					{{place.placeId}}  -  {{place.description}}			
				</li>
			
			</ul>
			
			<!-- <p>placeId: <input type="text" ng-model="placeForm.placeId" readonly></p> 
			<p>description: <input type="text" ng-model="placeForm.description"></p>
			 -->
			<button ng-click="insert()">Insert</button>
			<button ng-click="update()">Update</button>
			<button ng-click="del()">Delete</button>
