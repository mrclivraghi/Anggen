...
{
if (!$scope.enumValueDetailForm.$valid) return; 
enumEntityService.selectedEntity.show=false;projectService.selectedEntity.show=false;entityGroupService.selectedEntity.show=false;restrictionEntityGroupService.selectedEntity.show=false;roleService.selectedEntity.show=false;restrictionFieldService.selectedEntity.show=false;fieldService.selectedEntity.show=false;tabService.selectedEntity.show=false;enumFieldService.selectedEntity.show=false;annotationService.selectedEntity.show=false;relationshipService.selectedEntity.show=false;entityService.selectedEntity.show=false;restrictionEntityService.selectedEntity.show=false;annotationAttributeService.selectedEntity.show=false;userService.selectedEntity.show=false;enumValueService.update().then(function successCallback(response) { 
$scope.search();
},function errorCallback(response) { 
AlertError.init({selector: "#alertError"});
AlertError.show("Si è verificato un errore");
...
userService.selectedEntity.enumEntityList.push(response.data);
},function errorCallback(response) { 
AlertError.init({selector: "#alertError"});
AlertError.show("Si è verificato un errore");
return; 
...
for (i=0; i<userService.selectedEntity.enumEntityList.length; i++)

{

if (userService.selectedEntity.enumEntityList[i].enumEntityId==enumEntityService.selectedEntity.enumEntityId)

userService.selectedEntity.enumEntityList.splice(i,1);

}

userService.selectedEntity.enumEntityList.push(enumEntityService.selectedEntity);
