    ...
    extends CrudRepository<it.anggen.model.entity.Entity, java.lang.Long>
{
    ...
    @Query("select e from Entity e where  (:entityId is null or cast(:entityId as string)=cast(e.entityId as string)) and (:descendantMaxLevel is null or cast(:descendantMaxLevel as string)=cast(e.descendantMaxLevel as string)) and (:name is null or :name='' or cast(:name as string)=e.name) and (:securityType is null or cast(:securityType as string)=cast(e.securityType as string)) and (:relationship in elements(e.relationshipList)  or :relationship is null) and (:enumField in elements(e.enumFieldList)  or :enumField is null) and (:tab in elements(e.tabList)  or :tab is null) and (:restrictionEntity in elements(e.restrictionEntityList)  or :restrictionEntity is null) and (:entityGroup=e.entityGroup or :entityGroup is null) and (:field in elements(e.fieldList)  or :field is null) ")
    public List<it.anggen.model.entity.Entity> findByEntityIdAndDescendantMaxLevelAndNameAndSecurityTypeAndRelationshipAndEnumFieldAndTabAndRestrictionEntityAndEntityGroupAndField(
