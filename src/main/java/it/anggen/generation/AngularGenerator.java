...
public class AngularGenerator {
	...
	private void renderTabForm(HtmlCanvas html,Boolean search) throws IOException {
		...
		if (entityName.toLowerCase().equals("ordine"))
			System.out.println("");
		/*
		 * print ul
    ...
    <li role="presentation"><a href="#tab1" aria-controls="tab1" role="tab" data-toggle="tab">Tab1</a></li>
		...
		for (int i=0; i<tabList.size(); i++)
		{
			...
			List<EntityAttribute> entityAttributeList=entityManager.getFieldByTab(tab);
			...
			*/
			...
			/*if (!search)
				htmlTab._div();*/
			sbTab.append(htmlTab.toHtml());
			if (!search)
			{
				sbTab.append(" </uib-tab>");
			}
			if (filled)
	...
	private Boolean renderField(HtmlCanvas html,EntityAttribute entityAttribute, Boolean search,String style,String baseEntity,Integer tabIndex) throws IOException
	{
		...
		if (search && EntityAttributeManager.getInstance(entityAttribute).getPassword()) return false;
		if (search && EntityAttributeManager.getInstance(entityAttribute).isEmbedded()) return false;
