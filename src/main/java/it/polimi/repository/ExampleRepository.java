    ...
    extends CrudRepository<Example, Integer>
{
    ...
    @Query("select e from Example e where  (:exampleId is null or cast(:exampleId as string)=cast(e.exampleId as string)) and (:name is null or :name='' or cast(:name as string)=e.name) and (:eta is null or cast(:eta as string)=cast(e.eta as string)) and (:male is null or cast(:male as string)=cast(e.male as string)) and (:birthDate is null or cast(:birthDate as string)=cast(date(e.birthDate) as string)) and (:birthTime is null or cast(:birthTime as string)=cast(date_trunc('seconds',e.birthTime) as string)) ")
